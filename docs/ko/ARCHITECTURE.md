...
로그인 화면의 브라우저 전용 **ID/PW 저장** 체크박스는 이메일, Base64로 난독화한 비밀번호, 자동 로그인 선택 상태를 `localStorage`에 저장하고 자동 제출 없이 복원한다. 체크하지 않은 로그인에 성공하면 세 값을 모두 삭제한다. Base64는 암호화가 아니므로 화면에서 공용 PC 사용을 경고한다. 아래에 들여쓴 **자동 로그인**은 ID/PW 저장 전에는 비활성화되며, 자동 로그인 선택 시 ID/PW 저장도 선택되고 ID/PW 저장 해제 시 자동 로그인도 해제된다. 인증 API에는 `remember`만 전송한다. 로그아웃은 세션 쿠키만 삭제하고 저장된 자격 증명은 유지한다. IP+이메일 기준 15분 동안 5회 실패 잠금도 그대로 적용한다. 불변 unique 사용자 slug, 워커 환경 격리, 설정 workdir 제한은 구현되었다. 프로젝트 `workdir` 상대경로는 해당 프로젝트 루트 기준으로 해석하고(`.`은 프로젝트 루트), 미지정 시 `workspace` 디렉터리를 사용한다. 상대경로 탈출과 심볼릭 링크 탈출은 거부하며 절대경로는 data root 또는 설정된 allowlist 안이어야 한다. 거부 시 요청 경로와 허용 루트를 워커 로그와 리포트에 기록한다. write-only 시크릿 저장소와 #009 실행별 LLM 자격 주입은 계획 상태다.
...
프로젝트 전체화면 챗은 패널과 같은 단일 스크롤 컨테이너 구조를 사용한다. 뷰포트 높이의 챗 셸은 고정하고 메시지 목록만 스크롤하여 세션 헤더와 입력창을 고정하며 같은 최신 메시지 이동 동작을 공유한다.
...
재로드한 도구 호출은 말풍선 밖에서 연속 항목을 묶은 한 줄 접힘 요약 배지로 표시한다. 배지를 펼쳐도 서버가 만든 고정 요약만 보이며, 저장된 도구 JSON과 가져온 페이지 본문은 세션 API 경계를 넘지 않고 알 수 없는 메시지 역할은 렌더하지 않는다.
## LLM 헬스 모니터링과 채팅 표시
