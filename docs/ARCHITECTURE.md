...
Reloaded tool calls are rendered outside chat bubbles as consecutive groups of one-line, collapsed summary badges. Expanding a badge shows only the server-generated fixed summary; stored tool JSON and fetched page text never cross the session API boundary, and unknown message roles are not rendered.
